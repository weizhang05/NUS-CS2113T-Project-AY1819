...
The data encryption feature will make use of the Java `Cipher` class that provides the functionality of a cryptographic cipher for encryption and decryption.

All participants' private contact details are saved in the JSON file `addressbook.json` when the user exits the application. The data from this file is retrieved when the user starts the application. The data encryption feature encrypts the data saved in this JSON file just before the application exits, and decrypts it when the application starts up, such that participants' private details cannot be accessed between these two periods.

==== Design Consideration
_{to be completed in v1.3}_
...
|`* * *` |user |log in with a unique user type |have the correct access permissions to participants' data
