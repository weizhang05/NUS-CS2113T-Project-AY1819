...
// tag::listF/O[]
=== List Freshmen / OGL feature
The inidividual list commands to list freshmen, OGLs and all the participants in the contact list will allow for easy filtering through the list for the FOP Organizers. 

==== Current Implementation
The 'List Freshmen' and 'List Orientation Group Leader (OGL)' commands use the keywords (predicates) "Freshman" and "OGL" to parse through the all the tags of all the participants in the FOP Manager's contact list. All the participants with the respective matches will be added into the filtered participant list, enabling the user to view all the freshmen and all the OGLs respectively in the contact list.

==== Design Considerations
* **Alternative 1**: Create separate lists for the Freshmen, OGLs and all participants.
** Pros: Easy to call out the list.
** Cons: Need to create switch cases for `UI` to display the respective lists. Unnecessarily long codes to filter out freshmen and OGLs from the participant list.
* **Alternative 2**: Add a `tag` to the Freshmen and OGL when they are added by their respective Add commands
** Pros: Able to use a Predicate search for the keyword. Easy to implement and fewer lines of code. In the implementation of the Export feature, Excel spreadsheet will contain the "Freshmen" and "OGL" tags that will aid in the easy filtering on Excel spreadsheet as well.
** Cons: Too many tags to the participant added.
// end:: listF/O[]

// tag::export[]
=== [Proposed] Export Feature

The Export feature will allow the FOC Project Director to export the data into excel files and distribute to other relevant people involved in the FOC. This way, data can be distributed and raw data untouched except by the Project Director.
...
|`* * *` |user |change the name of a House or Group |give my groups and houses names later into the camp planning
