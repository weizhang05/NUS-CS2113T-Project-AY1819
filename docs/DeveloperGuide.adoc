...
|=========
|Alternatives |Pros | Cons
|Programmatically call the `edit` command |Changing of interface with the model only requires changing the EditCommand and Parser |Tightly coupled with EditCommand and Parser
|Interact with ModelManager (current choice) |Modification is obvious and follow guidelines of the architecture of the system and cleaner code |Coupled with ModelManager
|=========
Rationale: Choice is made based on design practice. It may seems very intuitive to programmatically call the `edit` command,
 but it might causes unknown complexity within the system if there is a change in the EditCommand or the parser.
...
|=========
|Alternatives |Pros | Cons
|Creating lists for OGLs and freshmen and assigning them list by list (current choice) |Ensures all groups will have at least an OGL and balanced distribution amongst the group |Requires more memory space
|=========

Rationale: There is only 1 consideration for this feature due to it's complexity, no other alternative has yet been identified.
Future developers are free to modify the algorithm but must ensure all groups has at least an OGL and freshmen are evenly distributed (with 1 or 2 difference in numbers for freshmen).
