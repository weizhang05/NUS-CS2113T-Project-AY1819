...
|`help` |Views help |`help` |<<Features,Viewing help>>
|`add_f` |Adds a freshman |`add_f n/NAME s/SEX b/BIRTHDAY p/PHONE e/EMAIL m/MAJOR g/GROUP [t/TAG]...` |<<Features,Adding a freshman>>
|`add_o` |Adds an organizer |`add_o n/NAME s/SEX b/BIRTHDAY p/PHONE e/EMAIL m/MAJOR g/GROUP [t/TAG]...` |<<Features,Adding an organizer>>
|`add_h` |Adds a house |`add_h HOUSENAME` |<<Features,Adding a house>>
|`add_g` |Adds a group |`add_g GROUPNAME HOUSENAME`|<<Features,Adding a group>>
|`list` |Lists all participants in contact list |`list` |<<Features,Listing all participants>>
|`edit` |Edits participant at `INDEX` |`edit INDEX [n/NAME] [p/PHONE_NUMBER] [e/EMAIL] [m/MAJOR] [g/GROUP] [t/TAG]...` |<<Features,Editing a Participant>>
|`find` |Finds participants by name |`find KEYWORD [MORE_KEYWORDS]` |<<Features,Locating participants by name>>
|`delete` |Deletes participant at `INDEX` |`delete INDEX` |<<Features,Deleting a participant>>
|`select` |Selects participant at `INDEX` |`select INDEX` |<<Features,Selecting a participant>>
|`history` |Lists all previously entered commands |`history` |<<Features,Listing entered comamnds>>
|`undo` |Undoes previous command |`undo` |<<Features,Undoing previous command>>
|`redo` |Redoes previously undone command |`redo` |<<Features,Redoing previously undone command>>
|`clear` |Clears contact list |`clear` |<<Features,Clearing all entries>>
|`exit` |Exits program |`exit` |<<Features,Exiting the program>>
|=============================================================
